...
class DAM4SAMTracker:
    def __init__(self, tracker_name="sam21pp-L", input_image_size=1024):
        """
        ...
        self.predictor = build_sam2_video_predictor(
            self.model_cfg, self.checkpoint, device="cuda:0", hydra_overrides_extra=[f"model.image_size={self.input_image_size}"]
        )
        self.tracking_times = []
    ...
    def track(self, image, init=False):
        ...
        for out in self.predictor.propagate_in_video(
            self.inference_state, start_frame_idx=self.frame_index, max_frame_num_to_track=0, return_all_masks=True
        ):
            if len(out) == 3:
                ...
                alternative_masks, out_all_ious = (
                    alternative_masks_ious  # Extract all predicted masks (chosen and alternatives) and IoUs
                )
                ...
                if len(self.object_sizes) > 1 and n_pixels >= 1:
                    obj_sizes_ratio = n_pixels / np.median([size for size in self.object_sizes[-300:] if size >= 1][-10:])
                else:
                    obj_sizes_ratio = -1
                ...
                if (
                    m_iou > 0.8
                    and obj_sizes_ratio >= 0.8
                    and obj_sizes_ratio <= 1.2
                    and n_pixels >= 1
                    and (self.frame_index - self.last_added > 5 or self.last_added == -1)
                ):
                    alternative_masks = [
                        Mask((m_[0][0] > 0.0).cpu().numpy())
                        .rasterize((0, 0, self.img_width - 1, self.img_height - 1))
                        .astype(np.uint8)
                    ...
                    alternative_masks = [
                        np.logical_and(m_, np.logical_not(chosen_mask_np)).astype(np.uint8) for m_ in alternative_masks
                    ]
    ...
    def estimate_mask_from_box(self, bbox):
        ...
        unnorm_box = _transforms.transform_boxes(box, normalize=True, orig_hw=(self.img_height, self.img_width))  # Bx2x2
        ...
        sparse_embeddings, dense_embeddings = self.predictor.sam_prompt_encoder(points=concat_points, boxes=None, masks=None)
        ...
        masks = _transforms.postprocess_masks(low_res_masks, (self.img_height, self.img_width))
